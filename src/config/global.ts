...
export function loadGlobalConfig(): GlobalConfig {
  ...
  return globalConfigSchema.parse(
    mergeGlobalConfig(defaultGlobalConfig(), parsed as Partial<GlobalConfig>),
  );
}
...
export function clusterRegistryName(name: string): string {
  return `k3d-${name}`;
}

export function clusterRegistryRef(config: GlobalConfig): string {
  return `${clusterRegistryName(config.registry.name)}:${config.registry.port}`;
}

export function hostRegistryRef(config: GlobalConfig): string {
